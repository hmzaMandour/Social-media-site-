...
export const Creators = () => {
    ...
    return (
        <div className='h-[450px] w-[300px] rounded-[20px] overflow-hidden '>
            ...
            <div onChange={(e) => setSearch(e.target.value)} className='w-[100%] flex justify-center items-center'><input placeholder='Search' className='h-[45px] w-[250px]  rounded-[20px] pl-3' type="search" /></div>
            {
                CreatorsData.creators.filter((item) => {
                    return search.toLowerCase() === '' ? item : item.name.toLowerCase().includes(search)
                }).map((e, i) => (
                    <div key={e.id} className='flex justify-center items-center flex-col gap-[12px] mt-2'>
                        <div className=' h-[75px] w-[95%] rounded-[10px] flex items-center'>
                            <div className='h-[45px] relative w-[45px] ml-5 rounded-full  flex items-center justify-center'>
                                <img src={e.image} alt="" className='h-[45px]  w-[45px] ml-5 rounded-full  flex items-center justify-center' />
